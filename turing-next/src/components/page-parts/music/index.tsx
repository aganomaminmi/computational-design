...
export const MusicSketch: React.FC<{ id: string }> = ({ id }) => {
  ...
  const fftDo = (p: p5) => {
    ...
    if (kick.isBeat) {
      mutation(p, p.random(0, canvas), p.random(0, canvas))
    }
  ...
  const preload = (p: p5) => {
    ...
    sound = p.loadSound(`/music-resource/${id}.m4a`) // 音声データを読み込む
  ...
  const mousePressed = (p: p5) => {
    mutation(p, p.mouseX, p.mouseY)
  }

  const mutation = (p: p5, _x: number, _y: number) => {
    const x = _x / m + 1
    const y = _y / m + 1
    for (let i = 1; i <= n; i++) {
      for (let j = 1; j <= n; j++) {
  ...
  return (
    <ReactP5Wrapper setup={setup} preload={preload} draw={draw} mousePressed={mousePressed} keyPressed={keyPressed} />
  )
}
